...
namespace FosMan {
    ...
    internal class FosParseRuleEvaluationTools : IDocParseRule<Fos> {
        ...
        public List<(Regex marker, int inlineGroupIdx)> StartMarkers { get; set; } = [
            (new(@"^\d+\.\s+(доклад)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(опрос)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(тестирование)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(курсовая\s+работа)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(практическая\s+работа)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(контрольная\s+работа)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(мини-кейсы)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(деловая\s+игра)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(эссе)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(реферат)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
            (new(@"^\d+\.\s+(эссе,\s+реферат)\.$", RegexOptions.Compiled | RegexOptions.IgnoreCase), 1),
        ];
                            ...
                            if (table.RowCount > 0 && table.Rows[0].Cells.Count >= 3) {
                                ...
                                for (var col = table.Rows[0].Cells.Count - 1; col >= 0; col--) {
                                    ...
                                    if (!string.IsNullOrEmpty(cellText) && cellText.StartsWith("коды", StringComparison.CurrentCultureIgnoreCase)) {
                                        fos.EvalTools.Add(evalTool);
                                        fos.EvalToolTables[evalTool] = (table, col);
                                        break;
                                    }
                                }
                            }
